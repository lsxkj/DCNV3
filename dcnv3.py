...
if isinstance(m, Detect):
    ...
    m.stride = torch.tensor(
        [s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s).to(torch.device('cuda')))]).cpu()  # forward
    self.model.cpu()
...
if isinstance(m, IAuxDetect):
    ...
    self.model.to(torch.device('cuda'))
    m.stride = torch.tensor(
        [s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s).to(torch.device('cuda')))[:4]]).cpu()  # forward
    self.model.cpu()
    # print(m.stride)
    check_anchor_order(m)
    m.anchors /= m.stride.view(-1, 1, 1)
